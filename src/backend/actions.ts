...
export async function handleBilledOperation<T>({ userId, actionKey, aiOperation }: { userId: string; actionKey: NicheFinderAcuActionKey; aiOperation: (cost: number) => Promise<T> }) {
    const walletRef = adminFirestore.collection('wallets').doc(userId);
